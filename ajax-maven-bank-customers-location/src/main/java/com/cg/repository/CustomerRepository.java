...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("SELECT NEW com.cg.model.dto.CustomerDTO (c.id, c.fullName, c.email, c.phone, c.locationRegion, c.balance) FROM Customer c WHERE c.email = ?1")
    Optional<CustomerDTO> findCustomerDTOByEmail(String email);

    @Query("SELECT NEW com.cg.model.dto.CustomerDTO (c.id, c.fullName, c.email, c.phone, c.locationRegion, c.balance) FROM Customer c WHERE c.email = ?1 AND c.id <> ?2 ")
